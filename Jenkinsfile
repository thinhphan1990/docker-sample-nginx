    ...
    stage('ssh-test') {
       steps 
       {
          withCredentials([sshUserPrivateKey(credentialsId: "SshToSever-test", keyFileVariable: 'keyfile')]) {
              ...
              sh "ssh -i ${keyfile} -o StrictHostKeyChecking=no root@192.168.0.16 \"docker-compose -f  dxs-o2o-backend/docker-compose.yml -f  dxs-o2o-backend/docker-compose.dev.yml up -d\""
